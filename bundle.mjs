...
var MEMORY_DIR = "/home/user/data/memory";
var CONVERSATIONS_FILE = join(MEMORY_DIR, "conversations.jsonl");
function initMemory() {
  mkdirSync(MEMORY_DIR, { recursive: true });
  if (!existsSync(CONVERSATIONS_FILE)) writeFileSync(CONVERSATIONS_FILE, "");
}
function saveTurn(userMsg, assistantMsg) {
  const entry = { timestamp: (/* @__PURE__ */ new Date()).toISOString(), user: userMsg, assistant: assistantMsg };
...
function getRecentConversations(limit = 5) {
  try {
    ...
    return lines.slice(-limit).map((l2) => JSON.parse(l2));
  } catch {
    return [];
  }
}
function getContext() {
  ...
  for (const c2 of recent) {
    parts.push(`[${c2.timestamp}] Task: ${c2.user.slice(0, 150)}`);
    parts.push(`Result: ${c2.assistant.slice(0, 200)}`);
  }
  return parts.join("\n");
}
...
var toolHandlers = {
  ...
  ls: ({ path } = {}) => {
    try {
      ...
      return entries.map((e2) => `${e2.isDirectory() ? "d" : "-"} ${e2.name}`).join("\n") || "(empty)";
    } catch (e2) {
      return `Error: ${e2.message}`;
    }
  },
...
var SYSTEM_PROMPT = `You are a coding agent. You live in a workspace at ${WORKSPACE}.
...
async function handleMessage(content, messageId) {
  ...
  const enrichedContent = memoryContext ? `## Memory context from past sessions
${memoryContext}
...
${content}` : content;
  ...
  while (true) {
    ...
    if (resp.stop_reason === "end_turn" || !resp.content.some((b2) => b2.type === "tool_use")) {
      const result = resp.content.filter((b2) => b2.type === "text").map((b2) => b2.text).join("") || "";
  ...
  rl.on("line", async (line) => {
    ...
    if (msg.type === "message") {
      ...
      try {
        ...
        send({ type: "error", error: e2.message, message_id: mid });
        send({ type: "response", content: `Something went wrong: ${e2.message}`, message_id: mid, done: true });
      }
    }
  });
}
